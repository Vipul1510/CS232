...
char *DeCrypt(char *ciphertext, char *pswd)
{
  ...
  struct key_attr key_i;
  int prf_int1;
  int i = 1;
  for (px = 1; px < PASS_Y_LIM * PASS_X_LIM; px++)
  {
    if (i == len)
    {
      i = 0;
    ...
    key_i = key[px];
    prf_int1 = (key_i.li ^ key_i.zi ^ key_i.ki) & (15);
    int sub_result = sub_func(prf_int1) ^ pswd[px];
    buff[i] ^= sub_result;
    i++;
  }
  return buff;
...
int CheckCRC(char *plaintext, int validCRC)
{
  ...
  if ((CalculateCRC(plaintext) == validCRC) && (!my_memcmp(plaintext, correct_plaintext, len)))
    return 1;
  return 0;
}
